...
int main(int argc,char** argv) {
    ...
    try {
        ...
        if(o.help) {std::puts("lumora_processing_benchmark --output FILE [--smoke | --sizes 512,1024,2048 --warm-up 100 --measured 500]");return 0;}
        if(const char* preload=std::getenv("LD_PRELOAD");preload && *preload) throw Error(5,"Ordinary benchmark requires no LD_PRELOAD instrumentation");
        ...
        root={{"schemaVersion",1},{"artifactType","lumora.processing.benchmark"},{"runStatus","incomplete"},{"complete",false},{"workload",QJsonObject{{"kind",qs(o.workload)},{"requestedSizes",sizes},{"warmUpFrames",integer(o.warmUp)},{"measuredFrames",integer(o.measured)}}},{"generatedUtc",utcNow()},{"rows",QJsonArray{}},{"failure",QJsonValue()}};
        writeProgress(output,root);for(auto size:o.sizes) for(const auto& id:rowIds()) {auto rows=root["rows"].toArray();rows.append(measureBenchmarkRow(id,size,o));root["rows"]=rows;writeProgress(output,root);}
