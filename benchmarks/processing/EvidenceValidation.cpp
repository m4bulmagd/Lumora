...
void validateBenchmark(const QJsonObject& root,bool v2) {
    ...
    for(const auto& value:rows) {
        ...
        validateProvenance(r["provenance"]);validateExecution(r["execution"]);validateResource(r["resourcePlan"],full,v2);check(natural(r["warmUpFrames"])==warm && natural(r["measuredFrames"])==measured,"Row count mismatch");
...
void validateAllocationRoot(const QJsonObject& root,bool v2) {
...
void validateArtifact(const std::string& bytes,const std::filesystem::path& base) {
    ...
    check(version==1 || (versioned && version==2),"Unsupported artifact schemaVersion");
    if(type=="lumora.processing.benchmark") validateBenchmark(root,version==2);
    else if(type=="lumora.processing.allocation-proof") validateAllocationRoot(root,version==2);
