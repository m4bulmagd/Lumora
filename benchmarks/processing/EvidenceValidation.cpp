...
void validateResource(const QJsonValue& value,bool full) {
    const auto r=object(value,"scope limitScope limitBytes requiredBytes fixedBytes threeOwnerReserveBytes gammaCacheReserveBytes candidateRequiredBytes boundedStatistics exclusions");check(r["scope"]==(full?"prepared_session":"standalone_stage"),"Resource scope mismatch");const auto limit=natural(r["limitBytes"],true),required=natural(r["requiredBytes"],true);strings(r["exclusions"]);
    if(full) {check(r["limitScope"]=="session_accounted_storage" && required<=limit,"Session exceeds budget");const auto fixed=natural(r["fixedBytes"]),reserve=natural(r["threeOwnerReserveBytes"]),gamma=natural(r["gammaCacheReserveBytes"]);(void)natural(r["candidateRequiredBytes"]);check(fixed<=required && reserve<=required-fixed && gamma==required-fixed-reserve,"Resource total inconsistency");const auto stats=object(r["boundedStatistics"],"externalSessionBytes processingPoolBytes displayPoolBytes frameObjectBytes orientationBytes engineStateBytes activationEnvelopeBytes actualRetainedStageBytes");for(const auto& v:stats) (void)natural(v);}
...
void validateInput(const QJsonValue& value) {const auto i=object(value,"patternId version seed sha256");check(i["patternId"]=="xorshift32_u16_v1" && natural(i["version"])==1 && natural(i["seed"])==0x6D2B79F5U,"Input pattern mismatch");hash(i["sha256"]);}
...
void validateBenchmark(const QJsonObject& root) {
    (void)object(root,"schemaVersion artifactType runStatus complete workload generatedUtc rows failure");const bool complete=boolean(root["complete"]);check(root["runStatus"]==(complete?"complete":"incomplete"),"Run status mismatch");timestamp(root["generatedUtc"]);
    ...
    const auto rows=array(root["rows"]);check(rows.size()<=sizes.size()*11 && (!complete || rows.size()==sizes.size()*11),"Incomplete or excessive row product");std::size_t index=0;
    for(const auto& value:rows) {
        ...
        const auto id=rowIds()[index%11];const auto size=static_cast<std::uint32_t>(natural(sizes[static_cast<qsizetype>(index/11)]));++index;const bool full=id.starts_with("full_standard_");
        ...
        check(r["pipelineDefinition"]==pipelineJson(full?processing::standardPipeline():diagnosticPipeline(id)),"Row pipeline mismatch");validateInput(r["input"]);
        check(r["sourceDescriptor"]==descriptorJson(monoFormat(),layoutFor(size,size)),"Source descriptor mismatch");const core::Orientation orientation{id=="full_standard_nonidentity",false,id=="full_standard_nonidentity"?core::Rotation::Degrees90:core::Rotation::Degrees0};check(r["orientation"]==orientationJson(orientation) && r["orientedDimensions"]==dimensions(size,size),"Orientation mismatch");
        validateProvenance(r["provenance"]);validateExecution(r["execution"]);validateResource(r["resourcePlan"],full);check(natural(r["warmUpFrames"])==warm && natural(r["measuredFrames"])==measured,"Row count mismatch");
...
void validateAllocationRoot(const QJsonObject& root) {
    (void)object(root,"schemaVersion artifactType runStatus complete smoke generatedUtc rows failure");const bool complete=boolean(root["complete"]),smoke=boolean(root["smoke"]);check(root["runStatus"]==(complete?"complete":"incomplete"),"Allocation run status mismatch");timestamp(root["generatedUtc"]);const auto rows=array(root["rows"]);check(rows.size()<=2 && (!complete || rows.size()==2),"Both allocation orientations required");int index=0;
    for(const auto& value:rows) {
        const auto r=object(value,"rowId pipelineDefinition input sourceDescriptor orientation orientedDimensions provenance execution resourcePlan warmUpFrames measuredCycles positiveControl measuredRegion heapTrace processingErrors drops complete");const bool oriented=index++!=0;check(r["rowId"]==(oriented?"full_standard_nonidentity":"full_standard_identity"),"Allocation row ordering mismatch");check(r["pipelineDefinition"]==pipelineJson(processing::standardPipeline()),"Allocation must execute shared Standard");validateInput(r["input"]);check(r["sourceDescriptor"]==descriptorJson(monoFormat(),layoutFor(64,48)),"Allocation source must be64x48");check(r["orientation"]==orientationJson({oriented,false,oriented?core::Rotation::Degrees90:core::Rotation::Degrees0}) && r["orientedDimensions"]==dimensions(oriented?48:64,oriented?64:48),"Allocation orientation mismatch");validateProvenance(r["provenance"]);validateExecution(r["execution"]);validateResource(r["resourcePlan"],true);const auto cycles=natural(r["measuredCycles"]);check(natural(r["warmUpFrames"])==(smoke?2U:100U) && cycles==(smoke?20U:1000U),"Allocation workload count mismatch");validateAllocation(r["positiveControl"],true,true);validateAllocation(r["measuredRegion"],true);check(natural(r["positiveControl"].toObject()["cycles"])==1 && natural(r["measuredRegion"].toObject()["cycles"])==cycles,"Allocation region cycles mismatch");validateHeap(r["heapTrace"],cycles,smoke);if(complete && !r["heapTrace"].isNull()) check(r["heapTrace"].toObject()["status"]=="complete","Complete allocation has failed trace");check(natural(r["processingErrors"])==0 && natural(r["drops"])==0 && boolean(r["complete"]),"Allocation processing failed");
    }
    validateFailure(root["failure"],static_cast<std::size_t>(rows.size()),complete);
}
...
void validateArtifact(const std::string& bytes,const std::filesystem::path& base) {
    const auto root=strictObject(bytes);check(natural(root["schemaVersion"])==1,"Unsupported artifact schemaVersion");const auto type=string(root["artifactType"]);
    if(type=="lumora.processing.benchmark") validateBenchmark(root);
    else if(type=="lumora.processing.allocation-proof") validateAllocationRoot(root);
