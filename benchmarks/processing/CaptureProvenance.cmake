...
include("${CMAKE_CURRENT_LIST_DIR}/SerializeBuildConfiguration.cmake")
lumora_serialize_build_configuration("${INPUT_DIR}" configured)
file(WRITE "${CONFIG_FILE}.tmp" "${configured}")
execute_process(COMMAND "${CMAKE_COMMAND}" -E copy_if_different "${CONFIG_FILE}.tmp" "${CONFIG_FILE}" COMMAND_ERROR_IS_FATAL ANY)
file(REMOVE "${CONFIG_FILE}.tmp")
...
# Avoid a fixed raw-string terminator that could also occur in a quoted flag.
set(raw_delimiter "evidence")
set(delimiter_index 0)
string(FIND "${configured}" ")${raw_delimiter}\"" terminator)
while(NOT terminator EQUAL -1)
  math(EXPR delimiter_index "${delimiter_index} + 1")
  set(raw_delimiter "evidence${delimiter_index}")
  string(FIND "${configured}" ")${raw_delimiter}\"" terminator)
endwhile()
file(WRITE "${OUTPUT_FILE}.tmp" "#pragma once\n#define EVIDENCE_REVISION \"${revision}\"\n#define EVIDENCE_DIRTY ${dirty}\n#define EVIDENCE_STATUS_SHA \"${status_hash}\"\n#define EVIDENCE_CAPTURED \"${captured}\"\n#define EVIDENCE_CONFIG R\"${raw_delimiter}(${configured})${raw_delimiter}\"\n")
