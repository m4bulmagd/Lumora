...
The owner authorized [CLAHE histogram and sharpening cost reduction](architecture/milestones/m08-clahe-sharpen-performance.md), followed by complete-pipeline remeasurement. Work starts from `0df7a98` on `perf/m08-clahe-sharpen-cost`, with preserved baseline binaries and passing affected Release checks. Exact pixels and allocation/ownership constraints remain binding. Implementation, review and new performance results are pending; this authorization does not close M8 or the deferred external gates.
