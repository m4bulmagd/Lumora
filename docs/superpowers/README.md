...
The [M3 acceptance record](../architecture/milestones/m03-camera-api-simulator.md) records passing Linux/GCC and Windows/MSVC Debug/Release CI at the exact merged source commit. M4 Tasks 1–3 are merged with matching cross-platform CI. Task 4's latest-frame presenter and non-shipping simulator harness are implemented and locally tested; its matching Windows CI, Windows Release stress, native Windows 11 visual/DPI checks, and separate M4 acceptance remain pending. Normal application live-pipeline composition remains M5.
