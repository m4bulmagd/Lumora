...
The user approved M5 development on 2026-09-07 under the recorded M4 manual-validation deferral; the current implementation scope is Task 1 (state machine and command mailbox). M4 is not fully accepted. The preflight is a contract, not M5 test evidence; matching Windows CI and affected native UI checks remain required for acceptance.
