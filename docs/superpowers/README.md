...
The [M7 execution record](../architecture/milestones/m07-preflight.md) records the owner-authorized simulator continuation while M4/M5 native Windows checks and M6 hardware work remain pending. Tasks 1–3 are implemented and independently reviewed on `feat/m07-high-bit-depth`: fixed-order configuration validation, deterministic normalization, window/level and the terminal Gray8 boundary. All six Processing CTest suites pass locally in Debug and Release at `0eeb0b3`. Task 4 is integrating the bounded engine with the existing live lifecycle. Matching Windows/MSVC evidence, full final verification and separate acceptance are still required.
