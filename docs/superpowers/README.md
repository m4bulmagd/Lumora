...
The [M4 preflight](../architecture/milestones/m04-preflight.md) and [M4 plan](plans/2026-04-25-m04-minimal-live-viewer.md) carry forward the reviewed implementation details: exact whole-image Fit outside manual zoom limits, logical-pixel 100%, the installed `minimal` Qt headless plugin, source-session reset, and paint-completion-based freshness. Product semantics are recorded first in PRD §19.7–19.8 and design §7/§8.2/§11.2. Only the recorded M4-to-M5 development exception changes the normal entry sequence; Windows visual, installer, hardware, and other release acceptance requirements are not waived.
...
The user approved M5 development on 2026-09-07 under the recorded M4 manual-validation deferral; [Task 1 (state machine and command mailbox) is implemented and task-reviewed locally](../architecture/milestones/m05-camera-state-mailbox.md). M4 is not fully accepted. The preflight is a contract, not M5 test evidence; matching Windows CI and affected native UI checks remain required for acceptance.
...
The owner subsequently approved [Task 3: frame processor and processing worker](../architecture/milestones/m05-processing-worker.md), now implemented and locally verified through the review fixes at `3c59b11`. Its 30/30 Linux Debug/Release results do not inherit Windows verification from the baseline. The later explicit push approval opened [PR #6](https://github.com/m4bulmagd/Lumora/pull/6) at `ccabaae`, followed by authorized local Tasks 4–5 development on a separate branch without waiting for CI. [Task 4](../architecture/milestones/m05-startup-preferences.md) is now task-reviewed and locally verified through `5b4bff8` with 33/33 Linux Debug/Release entries and all ten existing configuration-store cases restored. [Task 5](../architecture/milestones/m05-live-integration.md) supplies visible production simulator live composition and explicit startup controls; its checkpoint records implementation, review fixes and Linux verification separately from pending Windows/full M5 acceptance. The [launch guide](../development/build-linux.md#launch-the-desktop-application) describes the normal application's synthetic-video workflow.
