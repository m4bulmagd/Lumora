...
The [M7 execution record](../architecture/milestones/m07-preflight.md) records the owner-authorized simulator continuation while M4/M5 native Windows checks and M6 hardware work remain pending. Tasks 1–4 are implemented and independently task-reviewed on the unmerged `feat/m07-high-bit-depth` branch: configuration validation, deterministic normalization, window/level, terminal Gray8 mapping and the pooled live engine. At `aaf93f0`, local Debug/Release each passed 41/41 checks and the tests-OFF/Basler-OFF Release application build passed. Whole-branch review is approved with no required code changes. Matching Windows/MSVC evidence and separate acceptance remain required.
