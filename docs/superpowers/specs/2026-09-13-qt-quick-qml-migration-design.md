...
Status: **Selected UI direction; historical integration design, locally implemented through the QML-only workstation.** [ADR 0001](../../adr/0001-qt-quick-qml-frontend.md) records the owner's choice of Qt Quick/QML. The proposal below is retained as the migration rationale; current implementation status is recorded separately and does not claim milestone acceptance.

**Current continuation (2026-09-15):** The readiness assessment and handoff below are historical. The [QML-only workstation plan](../plans/2026-09-15-qml-only-workstation.md) is implemented at local commit `920b4f6`, from main `e4520dd` on unmerged, unpushed branch `codex/qml-foundation`. The sole normal `lumora_app` is QML; normal configuration no longer requires Widgets, while unique legacy tests remain behind an explicit option. Current parity includes every processing editor, stopped exposure/gain/FPS/ROI/format, per-camera preferences and operator/admin installation orientation, with existing C++ policy, services, pixels and frame ownership retained. Source manifest SHA-256 `5626179b29588aaab2547ba631f1e35eae9abb7231a948e6086fa6e53da87d1e` binds passing 65/65 Debug and Release suites, six 24/24 native runs, a 16/16 legacy-option check followed by restoring that option OFF, and a staged audit of 113 ELF objects with zero errors. The staged real-application workflow passes in 27.496 seconds; independent trace and visual reviews pass. Fullscreen, sidebar collapse and saved UI preferences remain future work; Windows distribution, hardware, performance and milestone acceptance gates remain open. See [PROGRESS](../../PROGRESS.md).
