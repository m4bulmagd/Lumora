...
## Compiler-policy references

Microsoft documents that runtime floating-environment access requires `fenv_access`, enabled by `/fp:strict`, and that strict mode suppresses contractions: [MSVC floating-point behavior](https://learn.microsoft.com/en-us/cpp/build/reference/fp-specify-floating-point-behavior?view=msvc-170). GCC documents the no-contraction and dynamic-rounding options: [GCC 15.2 optimization options](https://gcc.gnu.org/onlinedocs/gcc-15.2.0/gcc/Optimize-Options.html). Build-command, generated-code and exact arithmetic tests remain required; flags alone are not output evidence.
