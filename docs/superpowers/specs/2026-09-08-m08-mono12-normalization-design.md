...
The selected specialization is validated UInt16 storage with `sampleMaximum == 4095`, independently of descriptor name, packing provenance, or validBits alone. A private compile-time choice shares the existing pixel loop and makes 4095 a compiler-visible constant. Preserve the exact mathematical result `(value * 65535 + maximum / 2) / maximum` with uint64 arithmetic, including the ordered range check before each write. The implemented constant 4095 path evaluates `16*v + ((m + (m >> 12)) >> 12)` with `m = 15*v + 2048`; the shift term is exactly `floor((m-1)/4095)` over validated 0..4095. Direct constant-division forms still emitted a recurring GCC 15 Release DIV, so this proven bounded evaluation replaces the originally proposed literal division expression. The [performance record](../../architecture/milestones/m08-mono12-normalization-performance.md#exact-implementation-and-arithmetic-decision) gives its bounds and proof; the independent test oracle retains direct division. Verify all 4096 valid values with an independent integer oracle, boundary/error/canary behavior, generated Release arithmetic, useful repeated kernel gain, and full-pipeline hashes.
