...
The [M8 continuation contract](../../architecture/milestones/m08-continuation.md) defines prepared detail arithmetic, sigma/radius/threshold semantics and bounded CPU execution. OpenCV supplies pinned Gaussian coefficient preparation and comparison provenance; per-frame execution uses retained Lumora scratch. Prepared CLAHE preserves the pinned clipping/reflection/interpolation algorithm with locally owned workspace. Gaussian double-kernel output is not claimed bit-equivalent to OpenCV’s quantized U16 backend; reference records identify the actual implementation. Candidate configurations may be prepared separately on the control thread and atomically published; active scratch is never warmed/resized concurrently. Session extent/storage/orientation and resource-envelope changes require stopped replacement.
...
The repository stores small synthetic unsigned 16-bit patterns for exact algorithm assertions and properly anonymized images for regression coverage. Each fixture records valid bit depth and expected pipeline revision. Unpacking, normalization, window/level, inversion, right-angle orientation, display mapping, and Original capture use exact pixel comparison on Linux/GCC and Windows/MSVC. CLAHE, Gaussian/median denoise, and sharpening use reviewed per-algorithm maximum absolute error and image-difference thresholds across the two platforms. Each record identifies the pinned OpenCV or prepared CPU implementation that produced it. Repeatability/reference outputs are also recorded on the designated Windows release build. Dependency locking prevents accidental algorithm drift; any tolerance change requires explicit review.
