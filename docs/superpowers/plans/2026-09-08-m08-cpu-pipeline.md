...
**Files:** Create private `src/processing/src/PreparedCpuExecutor.hpp/.cpp`; modify `src/CMakeLists.txt`, `src/processing/src/PreparedRuntime.hpp`, `ProcessingPreparation.cpp`, `FrameProcessingEngine.cpp`, `src/processing/include/lumora/processing/ProcessingPreparation.hpp`; create `tests/unit/processing/PreparedCpuExecutorTests.cpp` and register it in `tests/CMakeLists.txt`; extend `PreparedRuntimeTests.cpp` and `FrameEngineAllocationTests.cpp` where lifecycle/admission behavior is consumed. Update resource serialization/validation/schema files under `benchmarks/processing/` and `tests/unit/processing/EvidenceArtifactTests.cpp` for new execution metadata.

**Interfaces:** Private executor constructor accepts `std::size_t slots` in [1,4]; `slots() const noexcept`; synchronous `run(std::size_t itemCount, void* context, void (*work)(void*, std::size_t slot, std::size_t begin, std::size_t end) noexcept)`. Final exception/return form must implement the design's full serial fallback on floating-environment setup failure. Publish the chosen exact signature in the task report for Task 4. `ProcessingPreparationOptions::cpuExecutionSlots` defaults to 4 and is frozen by the plan. Resource evidence names the admitted slots and helper count; fixed engine bytes include the executor once, with documented thread runtime/stack exclusions.
...
- [ ] Implement the fixed-storage executor with at most three threads, no dynamic queue/function wrapper, startup readiness, partial-construction cleanup, context publication/completion barriers and helper environment propagation. Verify failure cleanup with a narrow injectable thread-start seam only if needed; keep test utilities private.
...
- [ ] Add a real worker callback allocation positive control to the process-global allocation test; a subsequent repeated arithmetic dispatch must report zero calls/bytes/releases. Keep thread creation/destruction outside the measured region. Test the worker control independently rather than assuming the existing caller control proves it.
- [ ] Update strict resource artifacts and their schema/validation arithmetic together; distinguish accounted bytes from execution counts. Document runtime-managed stack/TLS/bookkeeping exclusions accurately.
...
- [ ] CLAHE: size one int32 histogram per slot with checked arithmetic, dispatch disjoint tile ranges, barrier, dispatch disjoint interpolation rows. Preserve bin count, clipping/CDF loop order and FE_TONEAREST validation before mutation.
...
**Interfaces:** Existing row helpers remain the stage boundary; scalar fallback remains available on non-x86-64. No public execution-backend change.
...
- [ ] Add independent exact coverage around SIMD lane tails, tiny borders, threshold ties, saturation, maximum kernel radius and unaligned rows. Demonstrate the coverage detects a meaningful rounding or tail mutation.
