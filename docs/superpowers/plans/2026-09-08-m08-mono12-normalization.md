...
- [x] Freeze clean after tools/source/build/provider/schema bindings. Run normal six-row Mono12 benchmark at100/500, normal portable/glibc allocation profiles at100/1000, with no concurrent heavy work and no preload during timings. Only claim larger-image glibc/helper participation if a separately labeled matching Mono12 probe ran; large normal benchmark rows already cover C++ counters.
- [x] Independently regenerate actual input PGM SHA256 from uint32 xorshift upper12 for512/1024/2048 and64×48; validate schemas, raw trace events and positive/empty/helper controls, full output SHA256/equal-count fingerprints, zero errors/drops/C++ counts, unchanged resources and all frozen hashes. Report actual median/P95/FPS for all six profiles, gains/regressions/variance and evidence scope. Repeat only if needed to interpret noisy results.
