...
## M9 Task 3 verification map (final verification pending)
...
These rows identify the implemented regression coverage, not completed final verification. The [Task 3 record](milestones/m09-compare.md#verification-record) retains the pending clean-source full Debug/Release/native checks and evidence location. Task 3 remains unpublished with hosted platform CI pending. No processing, performance, native Windows or hardware acceptance is supplied by a display-mode change.
