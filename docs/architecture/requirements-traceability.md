...
This matrix starts the evidence chain for the approved evaluation architecture. Linux and Windows results below refer to the implemented M1–M3 subset at `2c88ec90ab58e3e6719be5e236dc49388dbc72dd`, verified in Debug and Release. Later milestones extend these rows instead of replacing them.
...
| DES-7 | §7 Camera and viewer state machines | Simulator lifecycle and persistent-disconnect tests; application/viewer states remain M4–M5/M12 | Verified for device subset | Verified for device subset | Partial |
| DES-8 | §8 Threading model | Thread-confined camera port, cancellable clock waits and retrieval; worker orchestration/shutdown remains M5/M12 | Verified for primitives/device | Verified for primitives/device | Partial |
| DES-9 | §9 Frame memory ownership and bounded buffering | `Core.BufferPool`, `Core.Frame`, `Core.LatestValueSlot`, `Core.BoundedQueue`; camera publication/lease tests | Verified for primitives/device | Verified for primitives/device | M2–M3 subset; application pool sizing remains later work |
...
| DES-11 | §11 Rendering and workstation UI | `MainWindowSmoke`; live rendering and controls remain M4/M9 | Verified for shell only | Verified for shell only | Partial |
...
| DES-15 | §15 Reliability and failure policy | M3 timeout, cancellation, fault transaction, sequence and lease regressions; system recovery remains M12 | Verified for device subset | Verified for device subset | Partial |
| DES-16 | §16 Test strategy | M1–M3 verification maps below; integration, reference-image, performance, soak and hardware suites remain later milestones | 18 CTest entries pass in Debug/Release | 18 CTest entries pass in Debug/Release | Partial |
| DES-17 | §17 Milestones and acceptance criteria | Per-milestone plans and [M3 acceptance status](milestones/m03-camera-api-simulator.md#acceptance-evidence) | M1–M3 regression evidence | M1–M3 regression evidence | M3 accepted; later milestones unaccepted |
