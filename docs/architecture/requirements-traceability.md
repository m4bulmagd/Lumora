...
The [progress summary](../PROGRESS.md) identifies the current integrated implementation checkpoint. The historical M5 checkpoint is `f01b408`; its [merged verification](milestones/m05-live-integration.md#merged-main-verification-and-branch-cleanup-2026-09-07) records passing local and post-merge Linux/Windows Debug/Release checks. **M3 is accepted for engineering/evaluation**; the [M3 verification record](milestones/m03-camera-api-simulator.md#cross-platform-ci-verification) retains that accepted baseline. M4/M5 are not fully accepted. Earlier exact-source references below remain historical evidence; they are not claims that later integrated verification is missing. An implemented subset does not imply that its complete design section or a later milestone is accepted.
...
| DES-10 | §10 Processing pipeline | M7 validated configuration, U16 mapping and paired pooled engine; [M8 complete prepared execution](milestones/m08-continuation.md) and later performance work are merged | M7/M8 local and CI verification retained in milestone records | M7/M8 Debug/Release PR CI passed at recorded heads | M8 Tasks 1–5, including live composition, are merged; 30 FPS, designated Windows evidence and full acceptance remain pending |
| DES-11 | §11 Rendering and workstation UI | Completed Task 4, [QML-only workstation](milestones/qml-only-workstation.md), [Task 5 layout](milestones/qml-layout.md), then [compact controls](../superpowers/plans/2026-09-16-ui-simplification.md#verification-evidence--16-september-2026) at `ebcf5be` | Migration/layout have separate Debug/Release and native records; simplification records Debug 70/70 and native software 30/30 only | Earlier Task 4B PR CI passed; later source-specific Windows and native visual/DPI acceptance remain separate | QML and Task 5 implemented; custom-preset management UI and full M9 acceptance remain open; C-arm review is planned |
...
| DES-13 | §13 Configuration and presets | [Merged presets](milestones/m09-presets.md#pr-17-integration), [Task 4D per-camera/installation profiles](milestones/m09-camera-profiles.md), then [Task 4E user schema 5, machine schema 2 and capability fingerprint 2](milestones/m09-camera-controls.md) | Legacy review/migration, current readback and fixed/editable authority in full suites at `2be3a85`; targeted sanitizers at `ef28052` | Earlier preset PR CI passed; local Windows owner/ACL/UAC execution pending | Tasks 4D–4E implemented on local main; legacy data retained without authority upgrade; admission, activation and durable persistence remain distinct |
...
## Planned C-arm workstation extension (2026-09-17)

The owner requested the [C-arm design](../superpowers/specs/2026-09-17-carm-workstation-design.md) and [delivery phases](../superpowers/plans/2026-09-17-carm-workstation-roadmap.md). This table maps planned requirements only; no test execution or implementation is claimed.

| Requirements | Planned implementation / acceptance owner | Status |
|---|---|---|
| CARM-01–02 exact capture and session persistence | Existing M10 plus CR-1 in [capture/review plan](../superpowers/plans/2026-09-17-carm-capture-review.md) | Planned; pixel/recipe/identity and durability tests required |
| CARM-03–04 independent reference and saved-image processing | CR-2/CR-3 capture/review plan | Planned; ownership, resource and processing-isolation checks required |
| CARM-05–06 familiar UI and assigned displays | [UI plan](../superpowers/plans/2026-09-17-carm-workstation-ui.md), through CR-4 | Planned; native scenes, DPI/hotplug and operator tasks required |
| CARM-07 scene recording/replay | Delivery plan CR-5 | Planned; focused spec, admitted profile and exact-frame playback evidence required |
| CARM-08 truthful display/source states | Shared design and every affected phase | Planned extension of existing paused/stale behavior |
| CARM-09 event hold and inputs | Delivery plan CR-6 | Protocol/history/input gates unresolved |
| CARM-10 optional image tools | Delivery plan CR-7 | Individually scoped; no v1 exclusion silently removed |
| CARM-11 clinical records/interoperability | Delivery plan CR-8 | Separate clinical program; no real patient data authorized |
| CARM-12 input and workstation acceptance | Existing M6/M8/M12/M14 plus extension-specific verification | Existing gates remain open; no hardware evidence added |
...
The [Task 2 record](milestones/m09-processing-controls.md#pr-18-integration) retains its single Enhanced preview scope, final verification and PR #18 integration. [Task 3](milestones/m09-compare.md) extends presentation with mode switching and Compare and merged through PR #19 with its own local and hosted verification record.

## M9 Task 3 verification map

| Contract | Focused verification |
|---|---|
| Equal paired panes, literal distinct pixels, matching source/mapping/orientation, rejected-pair rollback and one completion receipt | `ComparisonViewport` |
| One transform, pointer-centered wheel zoom from either pane, shared pan/Fit/100%, odd widths and unchanged orientation | `ComparisonViewport`, existing `ImageViewport` and `ViewportTransform` |
| Token/mode/owner completion identity, same-bundle mode repaint without renewed freshness, paused switching, newest-frame resume, fallback/recovery and reset | `FramePresenter` |
| Exclusive mode intents, completed-mode labels/accessibility, availability reason and viewer-scoped shortcuts | `WorkstationView` |
| Actual MainWindow/camera/processing controls, paired pixels, PAUSED timestamp/age and control containment at 900×600 and 1280×800 | `CompareWorkstation` |

These regressions pass in the clean-source full Debug/Release suites at `f1848ec` (60/60 each). The [Task 3 record](milestones/m09-compare.md#verification-record) retains native X11 and actual Compare layout checks, exact timings, reviews and the verified evidence manifest. Receipt token/mode/owner guards were also reviewed directly; the tests do not independently falsify each guard using an injected wrong receipt. Task 3 subsequently merged through [PR #19](https://github.com/m4bulmagd/Lumora/pull/19), with Linux/Windows Debug/Release 60/60 each and Linux X11 1/1 each; the record preserves the first Linux timeout and passing bounded rerun. No processing, performance, native Windows or hardware acceptance is supplied by a display-mode change.


## M9 Task 4A camera settings verification map

| Test/check | Requirement covered | Evidence scope |
|---|---|---|
| `Application.CameraSettingsPolicy` | Exposure/gain-only compatibility; exact complete format, ROI, optional FPS and acquisition mode | Local and PR Task 4A policy tests; camera validation remains separate |
| `CameraSettingsDialog`, `CameraStartupPanel` | Real modeless draft widgets, capability modes/numbers, readback, selection/session/capability/revision invalidation, stopped/pending gating | Local/PR UI tests and inspected local native dialog layouts |
...
The [Task 4B record](milestones/m09-frame-rate.md#final-source-verification) binds these results to clean `8794c1c`: 62/62 in each configuration plus native X11 1/1 each. The precision P2 is closed; the nonblocking scripted-test acknowledgement P3 is retained. [PR #21 integration](milestones/m09-frame-rate.md#pr-21-integration) separately records fresh publication-head checks at `a5972f2` and passing Linux/Windows PR Debug/Release 62/62 plus Linux X11 1/1 each. The actual merge `322a1cf` has the same tested tree. Native Windows, hardware, performance and formal acceptance remain open.

## Milestone 9 Task 4D verification map

| Check | Requirement covered | Evidence boundary |
|---|---|---|
| `Application.StartupPreferences`, `Configuration.StartupPreferences`, `Configuration.PresetCodec` | Stable identity, canonical capabilities, schema 4 migration/legacy preservation, per-camera coalescing/capacity, independent selection and truthful durable revision | Full Debug/Release and targeted sanitizers on `10e1018` |
| `Configuration.InstallationProfiles` | Machine schema 1, required/fallback policy, asynchronous save, conflicts/refresh, backup/repair, read-only trust validation and administrator writes | 19 Linux cases; Windows implementation reviewed but not executed |
| `InstallationPipeline`, `InstallationController` | Save versus activation, same-device orientation/resources/IDs, fresh review/confirmation, priority/stale guards and saved Resume | Full Debug/Release and targeted sanitizers |
| `InstallationSettingsDialog`, `CameraSettingsDialog`, `FramePresenter`, `WorkstationView` | Operator/admin controls, asymmetric previews, delayed loading/local edits, painted/paused orientation truth | Full Debug/Release; native installation dialog 7/7 with five verified captures |

The [Task 4D record](milestones/m09-camera-profiles.md) binds commands, source review, logs, hashes, layout evidence and limitations. This is local engineering verification, with no hosted CI or milestone acceptance claim.

## Milestone 9 Task 4E verification map

| Check | Requirement covered | Evidence boundary |
|---|---|---|
| Camera contracts, simulator, acquisition and Apply planning | Independent mode/value access, optional gain, fixed FPS, authoritative current configuration, validation against fresh facts and rejected-write guards | Full Debug/Release suites at production `2be3a85` |
| Startup/profile codecs, controller and LivePipeline | User schema 5, machine schema 2, fingerprint 2; legacy review; repairable drafts; fresh Confirm/Start authority after Apply; priority and installation guards | Full Debug/Release at `2be3a85`; targeted ASan/UBSan at test-only `ef28052` |
| Camera panel/dialog and desktop smoke | Compact context actions, requested/actual disclosure, physical identity, independent camera/viewer status, absent/read-only controls, visible warnings and minimum layouts | 52 native UI tests plus one desktop smoke per Debug/Release at `2be3a85`; 18 inspected captures, nine pixel-identical pairs |
| Large-frame simulator cancellation recovery fixture | Recovery render gets the existing five-second large-frame budget; cancellation timing, frame-ID/lease checks and production deadlines remain unchanged | Only `SimulatedCameraTests.cpp` changes at `ef28052`; 59/59 simulator tests per Debug/Release and 22/22 targeted sanitizer groups with leak detection |

The [Task 4E record](milestones/m09-camera-controls.md#verification) binds full Linux Debug/Release **68/68 groups each** to production `2be3a85`. Its manifest verifies the test-only successor's one-file delta and unchanged production trees; full 68-group suites were not rerun at `ef28052`. Native X11 uses a matching supplemental Qt runtime under Xvfb. These development checks claim no hosted CI, native Windows, physical-camera, reference-performance or formal M9 acceptance. The subsequent [local-main integration at `e2de210`](milestones/m09-camera-controls.md#local-main-integration) was unpublished at that checkpoint. The combined camera/QML/layout work was later [published at `d97b269`](milestones/qml-publication.md); publication does not expand the original verification scope.

## Qt Quick/QML implementation and remaining evidence

[ADR 0001](../adr/0001-qt-quick-qml-frontend.md) and the [architecture amendment](../superpowers/specs/2026-04-25-xray-imaging-workstation-design.md) select the now-implemented Qt Quick/QML frontend. The original [migration proposal](../superpowers/specs/2026-09-13-qt-quick-qml-migration-design.md) is historical. The [renderer](milestones/qml-renderer-experiment.md), [integrated Live](milestones/qml-live.md), [QML-only](milestones/qml-only-workstation.md) and [layout](milestones/qml-layout.md) records cover DES-3/4 (presentation/build), DES-7/8/9 (freshness, Pause, receipts and ownership), DES-11 (controls/layout) and bounded deployment checks. Widgets tests alone do not establish Quick compatibility; the named records retain the Quick-specific evidence. Native Windows, physical hardware, performance and formal M13/release acceptance remain separate. The later `ebcf5be` compact-controls change records Linux Debug/software evidence only; all earlier results stay tied to their original source.
