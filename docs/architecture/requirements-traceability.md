...
| DES-11 | §11 Rendering and workstation UI | M4 viewer/startup sidebar; merged [M9 processing controls](milestones/m09-processing-controls.md#pr-18-integration) and [display modes/Compare](milestones/m09-compare.md#pr-19-integration) | Task 3 local and PR Debug/Release 60/60 plus X11 1/1 each; actual Compare layout passed | Task 3 PR Debug/Release 60/60 each; native visual/DPI pending | Tasks 2/3 merged through PR #18/#19; [Task 4A settings](milestones/m09-camera-settings.md#final-source-verification) local 62/62 plus X11 each at `0b6f8f4`, hosted CI pending; M4 unaccepted |
...
|---|---|---|
| `Application.CameraSettingsPolicy` | Exposure/gain-only compatibility; exact complete format, ROI, optional FPS and acquisition mode | Local Task 4A policy tests; camera validation remains separate |
| `CameraSettingsDialog`, `CameraStartupPanel` | Real modeless draft widgets, capability modes/numbers, readback, selection/session/capability/revision invalidation, stopped/pending gating | Local UI tests and inspected native dialog layouts |
| `LivePipeline` camera-settings cases | Full request Apply, unchanged resources/source continuity, fresh confirmation, invalid/stale admission, persistence/Resume, late load and Disconnect | Real controller/panel/dialog and production simulator integration |

The [milestone record](milestones/m09-camera-settings.md#final-source-verification) records clean source `0b6f8f4`: Debug/Release 62/62 plus native X11 1/1 each, independently approved source review and inspected 560×560/720×640 dialog captures. This partial Task 4 implementation does not cover low-FPS watchdog correction, format/ROI resource rebinding, per-camera records, administrator orientation or full M9 acceptance.
