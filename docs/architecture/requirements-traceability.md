...
The [Task 2 record](milestones/m09-processing-controls.md#pr-18-integration) retains its single Enhanced preview scope, final verification and PR #18 integration. [Task 3](milestones/m09-compare.md) extends presentation with mode switching and Compare; it has separate local development and verification status.
...
The [milestone record](milestones/m09-camera-settings.md) records final source and results. This partial Task 4 implementation does not cover low-FPS watchdog correction, format/ROI resource rebinding, per-camera records, administrator orientation or full M9 acceptance.
