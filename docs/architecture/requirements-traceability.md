...
| DES-11 | §11 Rendering and workstation UI | Merged M9 controls/Compare/settings through PR #21; [local-main integration of Tasks 4C–4E](milestones/m09-camera-controls.md#local-main-integration) includes compact camera controls and capability access | Full Debug/Release 68/68 and native UI/smoke at production `2be3a85`; simulator and sanitizer reruns at test-only `ef28052` | Earlier Task 4B PR CI passed; local Task 4C–4E Windows execution and native visual/DPI pending | Task 4 implemented on local main; QML migration selected but unimplemented; Task 5 and M9 acceptance remain open |
...
The [Task 4E record](milestones/m09-camera-controls.md#verification) binds full Linux Debug/Release **68/68 groups each** to production `2be3a85`. Its manifest verifies the test-only successor's one-file delta and unchanged production trees; full 68-group suites were not rerun at `ef28052`. Native X11 uses a matching supplemental Qt runtime under Xvfb. These development checks claim no hosted CI, native Windows, physical-camera, reference-performance or formal M9 acceptance. The subsequent [local-main integration at `e2de210`](milestones/m09-camera-controls.md#local-main-integration) is recorded separately and has not been pushed.

## Qt Quick/QML direction and pending evidence

[ADR 0001](../adr/0001-qt-quick-qml-frontend.md) and the [architecture amendment](../superpowers/specs/2026-04-25-xray-imaging-workstation-design.md) select Qt Quick/QML for the future frontend. The [migration proposal](../superpowers/specs/2026-09-13-qt-quick-qml-migration-design.md) links the next-session plan to DES-3/4 (presentation boundaries and build), DES-7/8/9 (freshness, Pause, render receipts and ownership), DES-11 (UI parity) and M13 deployment. These changes have no QML implementation or rendering evidence yet; existing Widgets tests do not establish Quick compatibility. All previous evidence remains tied to its recorded source.
