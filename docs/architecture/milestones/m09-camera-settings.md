...
At this Task 4A checkpoint, FPS, ROI, format and acquisition mode were read-only. The subsequent low-FPS watchdog correction and numeric stopped-FPS editing merged separately through [PR #21](m09-frame-rate.md#pr-21-integration); they are outside the Task 4A evidence above. Same-device resource rebinding, richer capability flags, per-camera records, administrator orientation and Task 5 fullscreen/preferences remain separate. The inherited lifecycle timeout, 30 FPS target, native Windows/hardware gates and formal M9 acceptance remain open; M9 is not complete.
