...
A single Enhanced preview makes the controls' effect visible. It falls back to Original when the current bundle lacks an Enhanced display. The label follows the actual completed paint; a paused Enhanced image retains its label across later processing failures. Pause retains the last painted bundle and its freshness/age semantics. Changes affect future frames and do not recompute the paused image. Original/Enhanced switching and Compare are recorded separately in [M9 Task 3](m09-compare.md).
...
Local evidence is under `out/qa/m09-controls/` in the preserved `.worktrees/m09-controls` checkout. `verification-manifest.json` verifies all seven successful command records, matching clean start/end source revisions and SHA-256 log hashes. The final screenshot is `processing-panel-final.bmp` (with a PNG format conversion for inspection). At this local checkpoint Task 2 was unpublished; subsequent publication, matching Linux/Windows Debug/Release CI and PR #18 integration are recorded below. The owner-approved [Task 3 continuation](m09-compare.md) has separate development evidence. No processing algorithm or performance measurement changed; M8 performance and deferred native Windows/hardware/acceptance gates remain open. The inherited intermittent lifecycle timeout is not claimed fixed by this work.
