...
On the M9 Task 2 controls branch, scroll to **Processing** after settings finish loading. Choose a preset or enable a stage and adjust its slider/numeric value. Accepted changes apply to subsequent frames and save in the background. **Reset processing** selects Original without changing the camera, pause or zoom. The single preview shows Enhanced when available and labels Original fallback only when that image is painted. A paused image stays frozen while settings change; resume the viewer to see new frames. Original/Enhanced switching and Compare are later work.
