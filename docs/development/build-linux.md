...
The old `linux-gcc-{debug,release}-sim-qml` presets remain compatibility names with separate build directories. They build the same sole `lumora_app`; `run-lumora-qml` also launches it. `lumora_qml_app` is a build-only compatibility target, not a second executable. The executable is now `out/build/<preset>/src/qml/lumora_app`; fresh Linux staging installs `bin/lumora_app`. Old build trees may still contain obsolete `src/lumora_app` (Widgets) or `lumora_qml_app` files. The `run-lumora` target and current `src/qml/lumora_app` path select the QML application.
...
Click or Tab to the image before Space=Pause/Resume, F=Fit, 1=100%, and +/-=zoom. These keys do not intercept numeric entry. F11 toggles fullscreen globally and Escape exits it, including from an open popup; layout transitions cancel unfinished processing text before moving focus. The compiled `Qml.Workstation` scene tests exercise real controls with the simulator; `Qml.Runtime`, `Qml.ProcessingAdapter`, `Qml.CameraSettingsAdapter` and `Qml.InstallationAdapter` cover their C++ adapters. `Qml.LayoutAdapter` covers window state, geometry and layout persistence. Actual fullscreen resizing needs a window manager; bare Xvfb does not manage window bounds. The layout verification uses isolated Openbox with Xvfb.
...
The **Processing** panel keeps Window/Level and Brightness/Contrast visible. Expand Gamma, Local Contrast, Denoise or Sharpen for additional adjustments. Drag a slider or click its value to enter an exact number; a valid edit enables that effect automatically. Rounded value previews are marked **≈**, while focus and tooltips show the exact value. Each effect's **…** menu can bypass it without discarding its settings. Opening a section changes visibility only. Accepted changes apply to subsequent frames and save in the background. **Reset** applies the Original processing preset without changing the camera, pause or zoom. A paused image stays frozen while settings change; resume the viewer to see new frames.

Once source configuration is confirmed, the setup sidebar collapses to give the image more space. **Source setup** reopens it, and required configuration review makes it visible again. Camera Stop/Disconnect remain available in either layout. Processing **Details** shows the acknowledged active recipe; the main **Details** control shows frame diagnostics.
