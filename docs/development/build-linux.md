...
```bash
LUMORA_QML_CAPTURE_DIR="$PWD/out/qa/qml-tone/captures-manual" \
...
The Linux-only `QmlPilot` install component stages the executable, scanned QML imports, runtime libraries/plugins and existing notices outside the build tree. See the [live checkpoint record](../architecture/milestones/qml-live.md) for deployment mechanics and the [tone editor record](../architecture/milestones/qml-tone.md) for the latest fresh stage. It is not an accepted installer or distribution bundle.
