...
A bundle without Enhanced falls back to Original and disables Enhanced/Compare with a visible reason. Recovery re-enables the buttons but leaves Original selected until the operator chooses a mode. A paused pair remains available despite later processing failures; Resume evaluates the newest bundle. The [Task 3 record](../architecture/milestones/m09-compare.md#verification-record) separates local development from final verification and hosted CI. Camera dialogs and fullscreen are subsequent work.
