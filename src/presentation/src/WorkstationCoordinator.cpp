...
struct WorkstationCoordinator::Impl {
    ...
    void loadPresets(const application::StartupPreferencesStatus& status) {
        if(presetsLoaded || !status.loadCompleted) return;
        presetsLoaded=true;
        processingPresentation.loadCompleted=true;
        auto showError=[&](const core::Error& error) {
            processingPresentation.loadError=error;
        };
        if(!status.loadedPresets) {
            if(status.warning) showError(*status.warning);
            else processingPresentation.loadError=core::Error{core::ErrorCategory::Configuration,
                "processing_settings_unavailable","Processing settings are unavailable.","",false};
            return;
        }
    ...
    void updateProcessing(const application::LivePipelineSnapshot& snapshot) {
        ...
        processingPresentation.persistenceWarning=std::move(warning);
    }

    std::optional<ContextHandoff> handoff;
    std::uint64_t nextHandoff{0};
    bool resumeStartWaiting{false};
    std::shared_ptr<application::LiveSessionContext> context;
    WorkstationState presentation;
    ...
    Result post(const application::CameraCommand& command, Intent intent) {
        ...
        if(result.hasValue()) {
            if(priority) {
                pending.reset(); resumePhase.reset(); resumeStartWaiting=false; barrier=command.requestId; barrierIntent=intent;
    ...
    Impl(application::LivePipeline& p,configuration::StartupPreferencesService& preferencesService,
        core::IClock& c,camera::CameraConfiguration request)
        :pipeline(p),preferences(preferencesService),clock(c),fixed(std::move(request)),desired(fixed) {
        presentation.requestedConfiguration=desired;
    }
};
WorkstationCoordinator::WorkstationCoordinator(application::LivePipeline& pipeline,
    configuration::StartupPreferencesService& preferences,core::IClock& clock,camera::CameraConfiguration fixed)
    :impl_(std::make_unique<Impl>(pipeline,preferences,clock,std::move(fixed))) {}
WorkstationCoordinator::~WorkstationCoordinator() { beginShutdown(); }
Result WorkstationCoordinator::start() { return dispatch(Intent::Refresh); }
void WorkstationCoordinator::poll() {
    auto& d=*impl_;
    if(d.stopped) return;
    auto snapshot=d.pipeline.snapshot();
    if(snapshot.context!=d.context && (!d.handoff || d.handoff->candidate!=snapshot.context)) {
        if(d.nextHandoff==std::numeric_limits<std::uint64_t>::max()) {
            d.presentation.controlsEnabled=false;
            d.presentation.startupWarning=rejected().error();
        } else d.handoff=ContextHandoff{++d.nextHandoff,snapshot.context};
    }
    d.presentation.contextBound=!d.handoff && snapshot.context && snapshot.context==d.context && snapshot.contextBound;
    d.processingPresentation.processorStatus=snapshot.processing.processorStatus;
    d.processingPresentation.retryPending=snapshot.processingRetryPending;
    ...
    if(d.resumePhase && currentCamera && currentCamera->sessionGeneration!=d.resumeGeneration &&
        ...
        d.resumePhase.reset(); d.resumeStartWaiting=false;
        d.pending.reset();
    }
    ...
    if(snapshot.error) {
        d.presentation.startupWarning=snapshot.error;d.pending.reset();d.barrier.reset();d.resumePhase.reset(); d.resumeStartWaiting=false;
        d.presentation.controlsEnabled=false;
    }
    ...
    d.presentation.ordinaryOperationPending=d.pending.has_value() || d.barrier.has_value() || d.installationBusy(snapshot) || d.resumeStartWaiting;
    d.presentation.resumeLiveAvailable=camera && d.eligible(*camera) && !d.presentation.startupWarning;
    if(continueResume==Intent::Start) {
        d.resumeStartWaiting=true;
        d.resumePhase=Intent::Start;
    }
    if(d.resumeStartWaiting) {
        if(!camera || !d.eligible(*camera) || !d.hasSuccessfulDesiredRequest(*camera) ||
            camera->sessionGeneration!=d.resumeGeneration || camera->appliedRevision!=d.resumeApplyRevision ||
            camera->confirmedRevision!=std::optional{d.resumeApplyRevision} ||
            !application::cameraConfigurationsEqual(camera->appliedConfiguration->actual,d.loaded->lastApplied) ||
            !d.installationBindingCurrent(snapshot) || d.barrier) {
            d.resumePhase.reset(); d.resumeStartWaiting=false;
        } else if(d.presentation.contextBound) {
            auto admitted=dispatch(Intent::Start);
            d.resumePhase.reset(); d.resumeStartWaiting=false;
            if(!admitted.hasValue()) d.presentation.startupWarning=admitted.error();
        }
    } else if(continueResume) {
    ...
    d.presentation.ordinaryOperationPending=d.pending.has_value() || d.barrier.has_value() ||
        d.installationBusy(snapshot) || d.resumeStartWaiting;
}
void WorkstationCoordinator::selectCamera(camera::CameraId id) {
    auto& d=*impl_;
    if(d.stopped) return;
    ...
    d.requestChosen=true; d.probeAttempted=true; d.resumePhase.reset(); d.resumeStartWaiting=false;
...
Result WorkstationCoordinator::applyCameraSettings(
    ...
    if(d.stopped || !d.presentation.controlsEnabled || d.pending || d.barrier || d.resumeStartWaiting)
...
Result WorkstationCoordinator::dispatch(Intent intent) {
    auto& d=*impl_;
    ...
    auto actionState=d.presentation;
    actionState.cameraStatus=snapshot.camera;
    actionState.ordinaryOperationPending=d.pending.has_value() || d.barrier.has_value() ||
        d.installationBusy(snapshot) || d.resumeStartWaiting;
    actionState.installationProfilePending=d.installationBusy(snapshot);
    actionState.installationBindingCurrent=d.installationBindingCurrent(snapshot);
    actionState.contextBound=!d.handoff && snapshot.context && snapshot.context==d.context && snapshot.contextBound;
    // A retained Resume Start is an authorized continuation, not ordinary work
    // admission blocking itself. Priority actions still see it above.
    if(!priority && d.resumeStartWaiting && intent==Intent::Start) actionState.ordinaryOperationPending=false;
    actionState.resumeLiveAvailable=snapshot.camera && d.eligible(*snapshot.camera) && !d.presentation.startupWarning;
    if(!CameraActionPolicy::evaluate(actionState).allows(intent)) return rejected();
...
const WorkstationState& WorkstationCoordinator::state() const noexcept { return impl_->presentation; }
const ProcessingPresentation& WorkstationCoordinator::processingState() const noexcept { return impl_->processingPresentation; }
ProcessingControlsModel* WorkstationCoordinator::processingControls() const noexcept { return impl_->processingModel.get(); }
std::optional<ContextHandoff> WorkstationCoordinator::pendingContextHandoff() const { return impl_->handoff; }
Result WorkstationCoordinator::completeContextHandoff(std::uint64_t id) {
    auto& d=*impl_;
    if(d.stopped || !d.handoff || d.handoff->id!=id) return rejected();
    const auto snapshot=d.pipeline.snapshot();
    if(snapshot.context!=d.handoff->candidate) return rejected();
    // Completion proves renderer retirement. Release our old owner before the
    // pipeline acknowledgement allows backend retirement of that same source.
    d.context=d.handoff->candidate;
    ...
    if(d.handoff->candidate) {
        ...
        if(!acknowledged.hasValue()) {
            ...
            d.presentation.startupWarning=acknowledged.error();
            return acknowledged;
        }
    }
    d.handoff.reset();
    const auto bound=d.pipeline.snapshot();
    d.presentation.contextBound=d.context && bound.context==d.context && bound.contextBound;
    return Result::success();
}
Result WorkstationCoordinator::beginCameraSettingsEdit(std::uint64_t generation,camera::CameraId id) {
    auto& d=*impl_;
    const auto snapshot=d.pipeline.snapshot();
    const auto& camera=snapshot.camera;
    if(d.stopped || !d.presentation.controlsEnabled || d.pending || d.barrier || d.resumeStartWaiting || d.installationBusy(snapshot) ||
        !camera || camera->state!=application::CameraSessionState::ConnectedIdle ||
        camera->sessionGeneration!=generation || camera->actualIdentity!=id || d.presentation.selectedCameraId!=id) return rejected();
    d.savedRequestReconciliationPending=false;
    d.requestChosen=true;
    d.settingsEditSource=std::pair{generation,std::move(id)};
    return Result::success();
}
Result WorkstationCoordinator::saveInstallationProfile(std::uint64_t generation,camera::CameraId id,
    core::Orientation orientation,bool confirmed,bool repair) {
    auto& d=*impl_;
    const auto snapshot=d.pipeline.snapshot();
    if(d.installationBusy(snapshot) || d.presentation.installationProfilePending ||
        d.installationRequest==std::numeric_limits<std::uint64_t>::max()) return rejected();
    ...
    d.resumePhase.reset(); d.resumeStartWaiting=false; d.presentation.resumeLiveAvailable=false;
...
Result WorkstationCoordinator::retryProcessing() {
    auto& d=*impl_;
    const auto snapshot=d.pipeline.snapshot();
    if(d.stopped || !d.presentation.controlsEnabled || !snapshot.context ||
        snapshot.context!=d.context || d.handoff || !snapshot.contextBound) return rejected();
    auto result=d.pipeline.requestProcessingRetry(snapshot.context->generation);
    if(result.hasValue()) {
        const auto updated=d.pipeline.snapshot();
        d.processingPresentation.processorStatus=updated.processing.processorStatus;
        d.processingPresentation.retryPending=updated.processingRetryPending;
    }
    return result;
}
void WorkstationCoordinator::completeRendererShutdown() noexcept {
    auto& d=*impl_;
    if(!d.stopped) return;
    d.handoff.reset();
    d.context.reset();
}
void WorkstationCoordinator::beginShutdown() noexcept {
    auto& d=*impl_;if(d.stopped) return;d.stopped=true;
    d.pending.reset();d.barrier.reset();d.resumePhase.reset(); d.resumeStartWaiting=false;
