...
[[nodiscard]] Result<void> validateProcessedFrame(
    const ImageLayout& layout, const SharedBuffer& pixels,
    const PipelineVersion& pipelineVersion, const ProcessingTimings& timings) {
...
[[nodiscard]] Result<void> validateDisplayFrame(
    const ImageLayout& layout, const SharedBuffer& pixels, DisplayStorage storage,
    const DisplayMapping& mapping, const Orientation& presentationOrientation) {
    const auto bufferValidation = validateBuffer(layout, pixels);
    if (!bufferValidation.hasValue()) {
        return Result<void>::failure(
            bufferValidation.error());
...
[[nodiscard]] Result<void> validateFrameBundle(
...
Result<std::shared_ptr<const ProcessedFrame>> ProcessedFrame::create(
    ...
    const auto validation = validateProcessedFrame(layout, pixels, pipelineVersion, timings);
    if (!validation.hasValue()) return Result<std::shared_ptr<const ProcessedFrame>>::failure(validation.error());
...
Result<std::shared_ptr<const ProcessedFrame>> ProcessedFrame::create(
    ...
    return detail::allocatePooledFrame<ProcessedFrame>(objects.state_, [&](void* storageAddress) {
        return new (storageAddress) ProcessedFrame(
            frameId,
            std::move(layout),
            std::move(pixels),
            pipelineVersion,
...
Result<std::shared_ptr<const DisplayFrame>> DisplayFrame::create(
    ...
    const auto validation = validateDisplayFrame(layout, pixels, storage, mapping, presentationOrientation);
    if (!validation.hasValue()) return Result<std::shared_ptr<const DisplayFrame>>::failure(validation.error());
...
Result<std::shared_ptr<const DisplayFrame>> DisplayFrame::create(
    ...
    return detail::allocatePooledFrame<DisplayFrame>(objects.state_, [&](void* storageAddress) {
        return new (storageAddress) DisplayFrame(
            frameId,
            std::move(layout),
            std::move(pixels),
            storage,
...
Result<std::shared_ptr<const FrameBundle>> FrameBundle::create(
    ...
    const auto validation = validateFrameBundle(raw, originalDisplay, enhanced, enhancedDisplay);
    if (!validation.hasValue()) return Result<std::shared_ptr<const FrameBundle>>::failure(validation.error());
...
Result<std::shared_ptr<const FrameBundle>> FrameBundle::create(
    ...
    return detail::allocatePooledFrame<FrameBundle>(objects.state_, [&](void* storageAddress) {
        return new (storageAddress) FrameBundle(
            std::move(raw),
            std::move(originalDisplay),
            std::move(enhanced),
            std::move(enhancedDisplay));
