...
The next bounded local [display-output optimization](docs/architecture/milestones/m08-display-output-performance.md) starts from clean `405c97b` and hoists validated immutable layout values out of the DisplayMapper and orientation hot loops at clean candidate `e801ece`. Formulas, 32×32 tiling, public interfaces, buffers, hooks, and Mono12/CanonicalU16/Gray8 behavior remain unchanged. Local Debug/Release pass 54/54, native X11 passes 1/1 in each configuration, targeted sanitizer checks pass 3/3, and independent source review is approved. In the matched Mono12 v3 100/500 run, 2048 identity/flip90 rises from **13.36/9.48 to 16.44/14.78 FPS**, while P95 falls from **96.8/116.0 to 71.4/75.7 ms**; exact outputs, resources, allocation proofs, and all 26 reference PGMs match. Unchanged control timings still vary, and independent final evidence audit is approved. The branch remains local; no publication, hosted Windows CI, or acceptance result is claimed.
